...
WhichTableToAdd DFA::getTableType(void) {
	...
	if (state == ADD || state == PUSH || state == LESS || state == MORE || state == NAME || state == NUMBER) {
		...
		if (symbol == LETTER) return KEYWORDS_OR_IDENTIFICATORS;
		if (symbol == DELIMETER || symbol == END_OF_LINE) return DELIMETERS;
...
vector <Token> DFA::parse(StaticTable &keyWords, StaticTable &delimiters,
	...
	while (this->automating()) {
		...
		if (this->isFinalState()) {
			...
			case KEYWORDS_OR_IDENTIFICATORS:

				if (keyWords.find(lex) == -1) {

					identificators.add(lex);
					token.tableName = "Identificators";
					token.firstPos = identificators.find(lex).first;
					token.secondPos = identificators.find(lex).second;
					token.lexem = lex;
